...
!      pHbulk = pHstart
      ...
      kp = Vnow
      VstepN = 1
      looper = (Vnow-Vend)*(Vstart-Vend) 
      do while(looper.ge.0) ! loop in pair strength
...
! Analysis
     ...
     &             *delta**3/vsol*zpol(im)
           ...
           if(infile.ne.5) then
              write(filename,'(A3, I3.3, A4)')'in.', VstepN, '.dat'
              open(unit=45, file=filename)
              do i = 1, (2+N_poorsol)*ncells
                 write(45, *) x1(i)
              enddo
           ...
           title = 'steps'
           call savetodisk(avpol_all, title, VstepN ,ccc)
...
!           looper = (pHbulk-pHend)*(pHstart-pHend)
!           pHstepN = pHstepN + 1
        endif

        if(rank.eq.1) then
           print*, 'pHbulk_rank1', pHbulk
           print*, 'Bulk composition / volume fracion rank 1'
           print*, 'Cations ', xposbulk
           print*, 'Anions  ', xnegbulk
           print*, 'H+      ', xHplusbulk
           print*, 'OH-     ', xOHminbulk
