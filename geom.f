       ...
       real*8 d2ring1, d2ring2, d2ring3
       real*8 xring1, xring2, xring3, zring1, zring2, zring3
       real*8 rring1, rring2, rring3
...
!------------------------ Hour glass, using part of half circle------
       ...
       rcurve = sqrt((dfloat(Dwall)/2*delta)**2
     & - ((dfloat(iZ-int((dimZ-Dwall)/2))-0.5)*delta
     & - dfloat(Dwall)/2*delta)**2)
       rcurve = CdimR*delta + dfloat(Dwall)/2*delta - rcurve
...
!------------------------ Hour glass, using part of half circle------
     ...
     & then
       ...
       zring1 = dimZ/2
       xring1 = CdimR-innerx/delta
       rring1 = innerr

       zring2 = dimZ/2+outerz/delta
       xring2 = CdimR-outerx/delta
       rring2 = outerr

       zring3 = dimZ/2-outerz/delta
       xring3 = CdimR-outerx/delta
       rring3 = outerr

       do iZ = 1, dimZ
       do iR = 1, dimR

       d2ring1 = ((dfloat(iZ)-0.5)-zring1)**2
     & + ((dfloat(iR)-0.5)-xring1)**2
       d2ring1 = sqrt(d2ring1)*delta

       d2ring2 = ((dfloat(iZ)-0.5)-zring2)**2
     & + ((dfloat(iR)-0.5)-xring2)**2
       d2ring2 = sqrt(d2ring2)*delta

       d2ring3 = ((dfloat(iZ)-0.5)-zring3)**2
     & + ((dfloat(iR)-0.5)-xring3)**2
       d2ring3 = sqrt(d2ring3)*delta

       if((d2ring1.le.rring1).or.(d2ring2.le.rring2)
     & .or.(d2ring3.le.rring3)) then
       matriz(iR, iZ) = -3
       ncells = ncells - 1
       endif
